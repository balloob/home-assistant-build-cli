...
func recipesForTopic(id string) []Recipe {
	...
	case "input-output":
		return []Recipe{{
			...
			RequiredCapabilities: []string{"local"},
			Inputs:               []RecipeInput{{Name: "command_path", Description: "Command path to inspect", Required: true, Source: "prompt"}, {Name: "action_name", Description: "Action/service name in domain.action format for service docs", Source: "prompt"}},
			Steps: []RecipeStep{
				{ID: "schema", Type: "discovery", Summary: "Inspect command schema and output contract", CommandTemplate: "hab schema {{command_path}} --json"},
				{ID: "docs", Type: "verification", Summary: "Inspect action schema for service calls when relevant", CommandTemplate: "hab action docs {{action_name}} --json", Notes: []string{"Only use when the target command is an action/service workflow."}},
			},
	...
	case "automation":
		return []Recipe{{
			...
			RequiredCapabilities: []string{"auth"},
			Inputs:               []RecipeInput{{Name: "action_name", Description: "Action/service in domain.action format", Required: true, Source: "prompt"}, {Name: "automation_id", Description: "Automation or script identifier", Required: true, Source: "prompt"}},
			Steps: []RecipeStep{
				{ID: "action-docs", Type: "discovery", Summary: "Inspect action/service contract", CommandTemplate: "hab action docs {{action_name}} --json"},
				{ID: "plan", Type: "discovery", Summary: "Preview automation mutation", CommandTemplate: "hab automation update {{automation_id}} -f automation.yaml --plan --json"},
	...
	case "calendar-todo":
		return []Recipe{{
			...
			Steps: []RecipeStep{
				...
				{ID: "add", Type: "mutation", Summary: "Add a new to-do item", CommandTemplate: "hab todo add {{todo_entity_id}} '{{summary}}' --json"},
	...
	case "operations":
		return []Recipe{{
			...
			Steps: []RecipeStep{
				...
				{ID: "backup-plan", Type: "discovery", Summary: "Preview a backup or maintenance mutation", CommandTemplate: "hab backup create \"Pre-maintenance\" --plan --json", Notes: []string{"Skip when the environment does not support backups."}},
